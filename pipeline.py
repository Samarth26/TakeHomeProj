...
plt.tight_layout()
plt.savefig("total_income_by_label.png", dpi=150)
...
def optuna_objective(trial, X, y, weights):
    ...
    trial.set_user_attr("gap", train_f1 - test_f1)
    return test_f1
...
def print_trials(study):
    ...
    print(df[["number", "value", "user_attrs_train_f1", "user_attrs_gap",
              "params_max_depth", "params_max_leaves", "params_n_estimators"]]
          .rename(columns={"value": "test_f1", "user_attrs_train_f1": "train_f1",
                            "user_attrs_gap": "gap"})
...
print(f"\nBest trial — test macro F1: {study.best_value:.4f}")
...
plt.xlabel(importance_type)
plt.tight_layout()
plt.savefig("feature_importance_colinearity_check.png", dpi=150)

# ── 14. Selected-feature model ────────────────────────────────────────────────
top8 = feature_importance.iloc[:-10].index.tolist()
extra = ["total_income"]
...
study_sel = optuna.create_study(direction="maximize")
...
print(f"\nBest trial — test macro F1: {study_sel.best_value:.4f}")
...
plt.xlabel(importance_type)
plt.tight_layout()
plt.savefig("feature_importance_selected.png", dpi=150)
